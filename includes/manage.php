<style>
	...
	<aside class="right-side">
		...
		<section class="content-header">
			...
			<ol class="breadcrumb">
				...
				<li><a href="?route=licenses">Clients new</a></li>
				<li class="active"><?php echo $client['name']; ?></li></ol>
		</section>
	 <div class="col-md-6 mrgndgh">
									<span class="fulwdth">
										<span class="t-label">Contract Qty:</span>
										<span class="t-number"><?php echo countTableFiltered('contract','clientid',$client['id'],'is_end',0);?></span></span>
										
										 <?php if($lia['role'] != 'technician'){ ?>
										 <span class="fulwdth">
										<span class="t-label2">Total Deposit:</span>
										<span class="t-number autoNumeric animated-number"><?php echo getTotalAmount('contract','deposit','clientid',$client['id']);?></span></span>
										<span class="fulwdth">
				...
				<div class="col-md-12">
					<div class="nav-tabs-custom">
						<ul class="nav nav-tabs">
							...
							<li <?php if ($section == "users") echo 'class="active"'; ?>><a href="#tab-users" id="go-users" data-toggle="tab">Users</a></li>
							...
							<li class="create_tic"<?php if ($_GET['tab'] == "department") echo 'class="active"'; ?> ><a href="#tab-department" data-toggle="tab">Create Tickets</a></li>
							<li <?php if ($section == "tickets") echo 'class="active"'; ?> ><a href="#tab-tickets" id="go-tasks" data-toggle="tab">Ticket List</a></li>
							<li <?php if ($section == "invoice" || $_GET['tab'] == "invoice") echo 'class="active"'; ?> ><a href="#tab-invoice" data-toggle="tab">Invoice </a></li>
							...
							<div class="btn-group pull-right" style="padding:6px;">
								...
								<a data-toggle='tooltip' title='New Contract' class="btn btn-primary btn-sm" href="?route=contract/create&clientid=<?php echo $client['id']?>&reroute=clients/manage&routeid=<?php echo $client['id']; ?>&section=contract" >New Contract</a>
								<a data-toggle='tooltip' title='New License' class="btn btn-default btn-sm" href="?route=licenses/create&clientid=<?php echo $client['id']; ?>">New License</a>
								<a data-toggle='tooltip' title='New Issue' class="btn btn-default btn-sm" href="#" onClick='showM("ajax.php?modal=issueAdd&reroute=clients/manage&routeid=<?php echo $client['id']; ?>&clientid=<?php echo $client['id']; ?>&section=issues");return false'><i class="fa fa-tasks"></i></a>
								<a data-toggle='tooltip' title='New Asset' class="btn btn-default btn-sm" href="?route=assets/create" ><i class="fa fa-barcode"></i></a>
								
								<a data-toggle='tooltip' title='New Project' class="btn btn-default btn-sm" href="#" onClick='showM("ajax.php?modal=projectAdd&reroute=clients/manage&routeid=<?php echo $client['id']; ?>&clientid=<?php echo $client['id']; ?>&section=projects");return false'><i class="fa fa-lightbulb-o"></i></a>
								<a data-toggle='tooltip' title='New Ticket' class="btn btn-default btn-sm" href="#" onClick='showM("ajax.php?modal=contractTicketAdd&reroute=clients/manage&routeid=<?php echo $client['id']; ?>&clientid=<?php echo $client['id']; ?>&contractid=&section=tickets");return false'><i class="fa fa-tag"></i></a>
								<a data-toggle='tooltip' title='New Credential' class="btn btn-default btn-sm" onClick='showM("ajax.php?modal=credentialAdd&reroute=clients/manage&routeid=<?php echo $client['id']; ?>&clientid=<?php echo $client['id']; ?>&section=credentials");return false'><i class="fa fa-asterisk"></i></a>
								<a data-toggle='tooltip' title='New User' class="btn btn-default btn-sm" onClick='showM("ajax.php?modal=userAdd&reroute=clients/manage&routeid=<?php echo $client['id']; ?>&clientid=<?php echo $client['id']; ?>&section=users");return false'><i class="fa fa-user"></i></a>
								...
								<a data-toggle='tooltip' title='Assign Admin' class="btn btn-default btn-sm" href="#" onClick='showM("ajax.php?modal=clientAssignAdmin&reroute=clients/manage&routeid=<?php echo $client['id']; ?>");return false'><i class="fa fa-thumb-tack"></i> </a></li>
							</div>
								 ...
								 <div class="table-responsive">
									<table id="dataTablesFull2" class="table table-striped table-hover">
										...
										<tbody>
											...
											echo "<tr>";
											echo "<td><a href='?route=assets/manage&id=".$license['id']."'>".$license['tag']."</a></td>";
									...
									<table id="dataTablesFullDesc2" class="table table-striped table-hover">
										...
										<tbody>
											...
											<tr>
												...
												<td><a href='?route=tickets/manage&id=<?php echo $ticket['id']; ?>'><?php echo getSingleValue("ticketsubject","subject",$ticket['subject']); ?></a></td>
												<td><?php if($ticket['adminid'] != 0) echo getSingleValue("people","name",$ticket['adminid']); else echo "<span class='text-muted'>Nobody</span>"; ?></td>
								<td><?php  if($ticket['clientid'] != 0) echo "<a href='?route=clients/manage&id=".$ticket['clientid']."'><span class='label' style=\"background-color:#FFF;color:#0073b7;border:1px solid #0073b7;\">" . getSingleValue("clients","name",$ticket['clientid']) . "</span></a> ";
	...
	<td><?php echo $ticket['consumable_cost'];?></td>
							...
							<div class="tab-pane<?php if ($section == 'users') echo 'active';?> " id="tab-users">
								 <div class="table-responsive">
									...
									<table id="dataTablesFull4" class="table table-striped table-hover">
										...
										//print_r($users);
											...
											echo "<td>
											<div class='pull-right'>
											<a href='?route=people/users/profile&id=".$user['id']."' class='btn-right text-dark'><i class='fa fa-eye'></i></a>&nbsp;
				...
				<div class="alert alert-info">  <i class="icon fa fa-info"></i> No files have been uploaded yet!  </div><?php } ?>
					 ...
					 <div class="pull-right">
						...
						<?php // die();  ?>
							<div class="tab-pane <?php if ($section == "" && $_GET['tab'] != "license"&& $_GET['tab'] != "department") echo 'active'; ?>" id="tab-contract">
			  ...
			  <option  value="<?= $newar; ?>"><?= $newar; ?>(<?php echo $vals[$newar] ; ?>)</option>
						<?php $i++;}  ?>
									 ...
									 <div class="col-md-3 w_full">
										 <strong>View</strong>
										 <select id="view-options" class="form-control select2" style="width:auto;display:inline;">
											  <option value="active" selected="">Active Contract</option>
											 
											 <option value="end">Ended Contract</option>
								   ...
								   <div class="col-lg-1 fll">
										<a class="btn btn-success btn-sm" id="print">Print Lable</a>
									</div>
							<!--a class="btn btn-success btn-sm" href="?route=printlable&id=<?php echo $client['id']; ?>">Print Lable</a-->
									<div class="col-lg-1 fll">
										<a class="btn btn-success btn-sm" href="#" onClick='showM("ajax.php?modal=issueInvoice&reroute=clients/manage&routeid=<?php echo $client['id']; ?>&clientid=<?php echo $client['id']; ?>&section=contract");return false'>Issue Invoice</a>
										<!--<a class="btn btn-success btn-sm" href="ajax.php?modal=issueInvoice&reroute=clients/manage&routeid=<?php echo $client['id']; ?>&clientid=<?php echo $client['id']; ?>&section=contract" data-toggle="modal" data-target="#myIssueModal">Issue Invoice</a>-->
									</div>
								</div>
								...
								<div id="all-contracts" class="table-responsive" style="display: none;">
									...
									<table id="dataTablesFullDesc6" class="table table-striped table-hover">
										<thead>
											<tr>
	...
	If it is applicable to this contract" class="circle-1"><i  class="fa fa-info-circle" aria-hidden="true"></i></span></th>
											 <?php  } ?>
	...
	<?php $asstevalw=getTableFiltered("assets","id",$contract['assetid']);
						...
						$serial_no = getSingleValue("assets","serial",$single['assetid']);
									...
									<span title="Serial No."> / <?php echo $serial_no; ?> </span><?php } ?></p>	
									<?php 
	...
	<div class="wrpnew">								
			...
			<div class="remaindays">
									...
									<p class="botm_style botm_styleppp"><span title="Contract No." class="contract_no_css contraddd"><?php echo $single['contractno']; ?></span></p>
			...
			class="fa fa-qrcode" aria-hidden="true"></i></span>
							...
							<?php	
								...
								<div class="row maindtext">
								 <?php  $deparrr =' ';?>
									<div class="head"><h3>
									<div class="col-lg-3 col-md-4 col-sm-4 bg_blue">
							
									<span style=" padding-left:0px;"><label>Department is not assigned</label></span></div></h3></div>	
									<?php  
	...
	<input type="hidden" name="reffer" value="<?php echo $_GET['id'];?>">
	<input type="hidden" name="sku" value="<?php echo $sku;?>">
	<textarea name="notes" class="summernote" id="notes"><?php echo $notes ;?></textarea>
	<input type="submit" class="submitbutton btn btn-success" value="Update">
	</form>
	...
	<span class="technician_info" title="<?php echo $tech_mailid . $tech_mobile;?>">( <?php echo $techi_name;?> )</span><?php } ?>
									...
									<div class="popstyle1 upcondep <?php echo $single['id']; ?>" style="display:none;">
											...
											<form method="POST">
												...
												<input type="hidden" name="reffer" value="<?php echo $_GET['id'];?>">
												<input type="hidden" class="textbox" name="conntde" value="<?php echo $depar; ?>">
												<?php  $depart = array_unique($contractsdepart_n);
												...
												<select class="select123" name="conntde">
											<?php
									  foreach($depart as $dip){
									  
					 $depar = getSingleValue("department","name",$dip);
	?>
			 <option <?php echo ($dip == $dipartmetn) ? 'selected' : '' ?> value="<?=$dip; ?>"><?= $depar; ?></option>
	...
	<p class="btnmb1 refresh1"><img class="rrdefv updatedepart" id="<?php echo $single['id']; ?>" width="18px" src="uploads/changing.png" alt="No Image"></p>
											<a class="btnmb1 sentto" href="#" onClick='showM("ajax.php?modal=ticketAdd&reroute=clients/manage&routeid=<?php echo $client['id']; ?>&contractid=<?php echo $single['id'] ;?>&section=tab-department");return false' data-toggle="modal"><img class="rrdefv"width="20px" style="" src="uploads/add_ticket.png" alt="No Image"></a>	
	
	
	<span class="btnmb1 limit">
	<span class="limit_icn rrdefv2" onClick="limit(<?php echo $single['id'];?>);" id="<?php echo $single['id'];?>"><i class="fa fa-tachometer" aria-hidden="true"></i></span>	
	<div class="limit_wrap " id="limit<?php echo $single['id'];?>">
	<i class="off_limiwrwp fa fa-times" aria-hidden="true"></i>
	...
	<select class="form-control select2 select2-hidden-accessible technician_value" onchange="techni(<?php echo $single['id'];?>,this.value)"id="technician_value" name="technician_value" style="width: 100%;" tabindex="-1" aria-hidden="true">
		...
		<input type='hidden' name='tech_mailid_data'  id='tech_mailid_data'value="<?php echo $emailval ;?>">
							...
							<div class="tab-pane <?php if ($section == "invoice") echo 'active'; ?>" id="tab-invoice">
								<div class="col-md-8">
									<span class="t-label">Total Amount:</span>
									<span class="t-number autoNumeric animated-number"><?php echo getTotalAmount('invoice','amount','clintid',$_GET['id']);?></span>
							
									<span class="t-label2">Total Paid:</span>
									<span class="t-number autoNumeric animated-number"><?php echo getTotalAmount('invoice','paid','clintid',$_GET['id']);?></span>
									
									<span class="t-label2">Total Balance:</span>
									<span class="t-number autoNumeric animated-number"><?php echo getTotalAmount('invoice','balance','clintid',$_GET['id']);?></span>
								...
								?>	
									<a class="btn btn-success btn-sm" href="?route=clients/invoice&id=<?php echo $client['id']; ?>">Issue Invoice</a>
								</div>
									...
									<table id="dataTablesFullDesc" class="table table-striped table-hover">
										...
										<tbody>
											...
											<tr>
												...
												<?php if($invoice['month']==1){ $month='Jan'; } elseif($invoice['month']==2){ $month='Feb'; } elseif($invoice['month']==3){ $month='Mar'; } elseif($invoice['month']==4){ $month='Apr'; } elseif($invoice['month']==5){ $month='May'; } elseif($invoice['month']==6){ $month='Jun'; } elseif($invoice['month']==7){ $month='Jul'; } elseif($invoice['month']==8){ $month='Aug'; } elseif($invoice['month']==9){ $month='Sep'; } elseif($invoice['month']==10){ $month='Oct'; } elseif($invoice['month']==11){ $month='Nov'; } elseif($invoice['month']==12){ $month='Dec'; } ?>
												<td><?php echo $month; ?></a></td>
												<td><?php echo $invoice['invoiceno']?></td>
												<td class="text-right"><?php echo number_format($invoice['amount'], 2, '.', ','); ?></td>
												
												<?php
												if($invoice['attachment']!= ""){
												$invo ="<a href='ajax.php?invoicefile=".$invoice['attachment']."'  title='file'><i class='fa fa-file'></i>" . $invoice['attachment'] . "<a><br>"; 
												}else{
												$invo='No Attachment';
												}
												?>
												...
												$invo_paid ="<a href='ajax.php?invoicefile=".$invoice['paid_file']."'  title='file'><i class='fa fa-file'></i>" . $invoice['paid_file'] . "<a><br>"; 
												}else{
												$invo='No Attachment';
												}
												?>

												<td><?php echo $invo_paid?></td>
												<td><?php echo $invoice['ornumber']?></td>
												<td class="text-right"><?php echo number_format($invoice['paid'], 2, '.', ','); ?></td>
												<?php
												if($invoice['or_file']!= ""){
												$invo_or ="<a href='ajax.php?invoicefile=".$invoice['or_file']."'  title='file'><i class='fa fa-file'></i>" . $invoice['or_file'] . "<a><br>"; 
												}else{
												$invo='No Attachment';
												}
												?>
												<td><?php echo $invo_or?></td>
												...
												<td>
													<div>
														...
														<a href='#' onClick='showM("ajax.php?modal=invoiceDelete&reroute=clients/manage&routeid=<?php echo $client['id']; ?>&id=<?php echo $invoice['id']; ?>&section=invoice");return false' class='btn-right text-red'><i class='fa fa-trash-o'></i></a>
													</div>
												</td>
		...
		$( "#dataTablesFullDesc6_filter input" ).addClass( "popopo" );
		  ...
		  var text = $(this).text();
			...
			var input = $('.popopo');
				input.val(newss);
				$('.popopo').keyup();
		  $("#tab-tickets").addClass("active");
		  $("#tab-department").removeClass("active");
	...
	$(".count_values_close").click(function(){
	...
	$(".contratc_active").click(function(){
	  ...
	  $('.updatedepart').click(function() {
			...
			$('.caseseriall').click(function() {
				var id = $(this).attr('id');
				alert=""
				$(".updtcssrl").not(".updtcssrl."+id).hide();
				$(".updtcssrl."+id).toggle();
			...
			$('#view-options').change(function() {
				...
				switch(v) {
					case 'all' :
						$('#active-contracts').hide();
						$('#ended-contracts').hide();
						$('#all-contracts').fadeIn();
						break;
					case 'active' :
						$('#active-contracts').fadeIn();
						$('#all-contracts').hide();
						$('#ended-contracts').hide();
						break;
					case 'end' :
						$('#ended-contracts').fadeIn();
						$('#all-contracts').hide();
						$('#active-contracts').hide();
						break;
				}
		...
		$('#mySelector').change( function(e) { 
			...
			if (letter === 'all') {
				...
				$("#dataTablesFullDesc6 tr td:nth-child(5)").each(
				function(){
					if($(this).text() != letter){
						$(this).parent().hide();
					}
		...
		$('.mySelector21').change( function(e) { 
		  ...
		  /* var i=0;
		   var str=document.getElementById("sami123").innerHTML;
		   var x= */
			var letter = $(this).val();
			//alert(letter);
			if (letter === 'all') {
		  ...
		  $("#dataTablesFullDesc6 tr td:nth-child(2)").each(
			  //i++;
				function(){
					if($(this).text() != letter){
						$(this).parent().hide();
		...
		$('#print').click(function() {
			//alert('grggth');
			jQuery("table#submittt").trigger("click");
			$("#submittt").click(); 
			return false;
		});
