...
function getTableFiltereddc($table, $filterColumn1, $filterValue1, $filterColumn2 = "", $filterValue2 = "", $columns = "*", $sortby = "id", $sortway = "DESC")
{ //get entire table filtered
    global $database;
    if ($filterColumn2 == "") {
...
function editClient($data)
{
    ...
    $database->update("clients", [
        "name" => $data['name'], "contact_person" => $data['contact_person'], "contact_no" => $data['contact_no'], "client_memo" => $data['client_memo'], "tech_incharge" => $data['technician_name'], "is_active" => $is_active, "it_manager_email" => $data['it_manager_email'],
...
function addContract($data, $files)
{
    ...
    $database->update("assets", [
        "clientid" => $data['clientid'], "status" => "In Service"
    ], ["id" => $data['assetid']]);
...
{ //update config value
    global $database;
    if($data['id']) {
        $database->update("invoiceTemplate", ["subject" => $data['subject'], "message" => $data['message'], "sms" => $data['sms']], ["id" => $data['id']]);
        return 40;
    } else {
        $database->insert("invoiceTemplate", [
            "subject" =>  $data['subject'],
            "message" => $data['message'],
            "sms" => $data['sms'],
        ]);
...
function sendEmail($to, $subject, $message, $clientid = "0", $peopleid = "0", $file = "",$inv_file="")
{ //send email
    // echo'<pre>';
    // print_r($subject);
    // echo'</pre>';
